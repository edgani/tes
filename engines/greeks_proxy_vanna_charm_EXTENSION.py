...
# ============================================================================
# CEM KARSAN VANNA + CHARM FLOW EXTENSION (v27.2)
# ============================================================================

def _vanna_proxy(ticker: str, prices, vix: float, dxy_ret: float = 0.0) -> dict:
    ...
    try:
        ...
        if vanna_score > 2.0:
            regime = "POSITIVE_VANNA"; note = "IV drop → dealer buyback → self-fulfilling rally"
        elif vanna_score < -2.0:
            regime = "NEGATIVE_VANNA"; note = "IV spike → dealer sell → pressure"
        else:
            regime = "NEUTRAL_VANNA"; note = "Vanna flows balanced"
        return {
            "ok": True, "vanna_score": round(vanna_score, 2), "iv_proxy": round(iv_proxy, 2),
            "iv_change": round(iv_chg, 2), "delta_shift": round(delta_shift, 4),
...
def _charm_proxy(ticker: str, prices, days_to_expiry: int = 7) -> dict:
    ...
    try:
        ...
        if charm_flow > 1.5:
            regime = "CHARM_BUY"; note = "Delta decay → dealer unwind shorts → drift higher into expiry"
        elif charm_flow < -1.5:
            regime = "CHARM_SELL"; note = "Delta decay → dealer unwind longs → drift lower into expiry"
        else:
            regime = "CHARM_NEUTRAL"; note = "Minimal expiration drift"
        return {
            "ok": True, "charm_flow": round(charm_flow, 2), "time_factor": round(time_factor, 2),
            "days_to_expiry": days_to_expiry, "regime": regime, "note": note,
            "color": "#3FB950" if "BUY" in regime else "#F85149" if "SELL" in regime else "#8B949E",
        }
    except Exception as e:
        return {"ok": False, "error": str(e)}

def get_vanna_charm_flows(ticker: str, prices, vix: float = 20.0, dxy_ret: float = 0.0, days_to_expiry: int = 7) -> dict:
    ...
    if combined_score > 2.5:
        signal = "NEVER_SHORT_EXPIRATION"; color = "#3FB950"
    elif combined_score < -2.5:
        signal = "AVOID_LONG_EXPIRATION"; color = "#F85149"
    else:
        signal = "NEUTRAL"; color = "#8B949E"
    return {
        "vanna": vanna, "charm": charm,
        "combined_score": round(combined_score, 2),
        "combined_signal": signal, "combined_color": color,
