"""crypto.py — Crypto Tab v40.8 (Keith Filtered + Fallback)

Renders tickers dari snap["crypto"] — pakai Keith filtered kalau ada, kalau tidak fallback ke semua.
"""
...
def render(snap: dict):
    ...
    keith_filtered = snap.get("crypto_keith_filtered", [])
    keith_meta = snap.get("crypto_keith_meta", {})

    # Get all tickers for this tab
    tab_data = snap.get("crypto", {})
