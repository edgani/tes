...
def _get_broker_proxy(ticker, prices):
    """
    AUDITED v40 — Proxy broker summary for IHSG with manipulation detection.
    Detects: real accumulation, real distribution, crossing (wash trading), cornering supply.
    """
    s = prices.get(ticker)
    if s is None or (hasattr(s, "__len__") and len(s) < 30):
        return {"real_accumulation": False, "real_distribution": False, "crossing_detected": False, "cornering_supply": False, "confidence": 0}
    try:
        s_clean = pd.to_numeric(pd.Series(s), errors="coerce").dropna()
        if len(s_clean) < 30: return {"real_accumulation": False, "real_distribution": False, "crossing_detected": False, "cornering_supply": False, "confidence": 0}
        ...
        # Real accumulation: price up + volume up + not crossing
        real_acc = False
        if r5d > 0.03 and r20d > 0.05 and not crossing:
            real_acc = True

        # Real distribution: price down + volume up + not crossing
        real_dist = False
        if r5d < -0.03 and r20d < -0.05 and not crossing:
            real_dist = True

        # Cornering supply: volume dries up then spike, tight range control
        cornering = False
        if len(s_clean) >= 10:
            vol_10 = float(s_clean.tail(10).std())
            vol_prev_5 = float(s_clean.iloc[-10:-5].std()) if len(s_clean) >= 10 else vol_20
            if vol_prev_5 < vol_20 * 0.5 and vol_5 > vol_20 * 1.2 and abs(r5d) < 0.05:
                cornering = True
...
# Basis line
        ...
        # ── Options Intel (Tier1 Alpha + NVTS) ──
        if market_type != "ihsg":
            intel_html = _render_options_intel(row)
            if intel_html:
                st.markdown(intel_html, unsafe_allow_html=True)

        # ── Bandarmology (IHSG only) ──
        if market_type == "ihsg":
            bandar_html = _render_ihsg_bandarmology(row)
            if bandar_html:
                st.markdown(bandar_html, unsafe_allow_html=True)
...
# ═══════════════════════════════════════════════════════════════════
# OPTIONS INTEL PANEL — Tier1 Alpha + NVTS Context
# ═══════════════════════════════════════════════════════════════════
def _render_options_intel(row):
    """Render deep options intelligence panel inside ticker expander.
    Context: Tier1 Alpha Market Structure + NVTS Vanna/Charm mechanics.
    """
    ticker = row.get("ticker", "?")
    ...
    iv_rank = opts.get("iv_rank")
    expected_move = opts.get("expected_move_pct")
    oi_call = opts.get("oi_call")
    oi_put = opts.get("oi_put")
    pc_ratio = opts.get("pc_ratio")
    next_expiry = opts.get("next_expiry")
    days_to_expiry = opts.get("days_to_expiry")
    source = opts.get("source", "PROXY")

    html = '<div class="ts-panel" style="border-color: #58A6FF40; margin-bottom: 8px;">'
    html += '<div class="ts-panel-title">📊 Options Intel — Tier1 Alpha Context</div>'

    # ── Market Structure Summary (Tier1 Alpha style) ──
    html += '<div style="background:#0D1117;border-radius:6px;padding:8px 10px;margin-bottom:8px;">'
    html += '<div style="font-size:0.6rem;color:#8B949E;text-transform:uppercase;font-weight:600;margin-bottom:4px;">Market Structure</div>'

    if gamma in ("POSITIVE", "DEEP_POSITIVE"):
        html += '<div style="font-size:0.72rem;color:#3FB950;margin-bottom:2px;">🟢 Gamma Exposure: POSITIVE — Dealers LONG gamma. Lower volatility expected. Rallies face call-wall resistance. Sell into strength / buy dips.</div>'
    elif gamma in ("NEGATIVE", "DEEP_NEGATIVE"):
        html += '<div style="font-size:0.72rem;color:#F85149;margin-bottom:2px;">🔴 Gamma Exposure: NEGATIVE — Dealers SHORT gamma. Trend ACCELERATION on breakout. Dips get bought. Buy weakness, don\'t short.</div>'
    else:
        html += '<div style="font-size:0.72rem;color:#D29922;margin-bottom:2px;">🟡 Gamma Exposure: TRANSITION — Directional play valid. Watch for regime shift.</div>'
    ...
    if vanna is not None and charm is not None:
        try:
            ...
            if abs(v) > 0.5 and abs(c) > 0.5:
                if (v > 0 and c > 0) or (v < 0 and c < 0):
                    html += '<div style="font-size:0.72rem;color:#3FB950;margin-bottom:2px;">✅ Systematic Flow: ALIGNED — Vanna+Charm sync. Stable dealer hedging flow.</div>'
                else:
                    html += '<div style="font-size:0.72rem;color:#D29922;margin-bottom:2px;">⚠️ Systematic Flow: MIXED — Vanna/Charm conflict. Expect choppy price action.</div>'
            else:
                html += '<div style="font-size:0.72rem;color:#8B949E;margin-bottom:2px;">⚪ Systematic Flow: NEUTRAL — No extreme vanna/charm detected.</div>'
        except:
            pass

    if direction == "LONG" and gamma in ("NEGATIVE", "DEEP_NEGATIVE"):
        html += '<div style="font-size:0.72rem;color:#3FB950;margin-bottom:2px;">🚀 Strategic Allocation: RISK ON — Negative gamma + long bias = momentum fuel.</div>'
    elif direction == "LONG" and gamma in ("POSITIVE", "DEEP_POSITIVE"):
        html += '<div style="font-size:0.72rem;color:#D29922;margin-bottom:2px;">🛡️ Strategic Allocation: NEUTRAL — Positive gamma + long bias = range-bound. Sell covered calls.</div>'
    elif direction == "SHORT":
        html += '<div style="font-size:0.72rem;color:#F85149;margin-bottom:2px;">🔻 Strategic Allocation: RISK OFF — Short bias. Fade rallies or buy puts.</div>'
    else:
        html += '<div style="font-size:0.72rem;color:#8B949E;margin-bottom:2px;">➡️ Strategic Allocation: NEUTRAL — No strong directional edge.</div>'
    html += '</div>'

    # ── Key Levels & Strikes (PV Band style) ──
    html += '<div style="background:#0D1117;border-radius:6px;padding:8px 10px;margin-bottom:8px;">'
    html += '<div style="font-size:0.6rem;color:#8B949E;text-transform:uppercase;font-weight:600;margin-bottom:4px;">Key Levels & Strikes</div>'
    levels = []
    if cw: levels.append(f'<span style="color:#F85149;">Upper PV/Call Wall: {_ffm(cw, market_type)}</span>')
    if pw: levels.append(f'<span style="color:#3FB950;">Lower PV/Put Wall: {_ffm(pw, market_type)}</span>')
    if mp: levels.append(f'<span style="color:#E6EDF3;">Max Pain: {_ffm(mp, market_type)}</span>')
    if entry: levels.append(f'<span style="color:#58A6FF;">Entry: {_ffm(entry, market_type)}</span>')
    if target: levels.append(f'<span style="color:#3FB950;">Target: {_ffm(target, market_type)}</span>')
    if stop: levels.append(f'<span style="color:#F85149;">Stop: {_ffm(stop, market_type)}</span>')
    if levels:
        html += '<div style="font-size:0.72rem;color:#E6EDF3;">' + ' · '.join(levels) + '</div>'
    if mp and px:
        mp_dist = (px - mp) / mp * 100
        html += f'<div style="font-size:0.68rem;color:#484F58;margin-top:2px;">Price vs Max Pain: {mp_dist:+.1f}% · {"Above" if mp_dist>0 else "Below"} pain = {"dealer sells rallies" if mp_dist>0 and gamma in ("POSITIVE","DEEP_POSITIVE") else "dealer buys dips" if mp_dist<0 and gamma in ("NEGATIVE","DEEP_NEGATIVE") else "transition zone"}</div>'
    html += '</div>'

    # ── Vanna & Charm Schedule (NVTS Context) ──
    html += '<div style="background:#0D1117;border-radius:6px;padding:8px 10px;margin-bottom:8px;">'
    html += '<div style="font-size:0.6rem;color:#8B949E;text-transform:uppercase;font-weight:600;margin-bottom:4px;">Vanna & Charm Schedule (NVTS Model)</div>'

    if next_expiry and days_to_expiry:
        html += f'<div style="font-size:0.72rem;color:#E6EDF3;margin-bottom:2px;">📅 Next Expiry: <b>{next_expiry}</b> · DTE: <b>{days_to_expiry}</b> days</div>'
        if days_to_expiry <= 5:
            html += '<div style="font-size:0.7rem;color:#F85149;margin-bottom:2px;">🔥 CHARM PEAK — Last days before expiry. OTM delta rising daily. Dealer forced to buy shares to hedge. ACCELERATION risk highest.</div>'
        elif days_to_expiry <= 10:
            html += '<div style="font-size:0.7rem;color:#D29922;margin-bottom:2px;">⚡ CHARM WINDOW ACTIVE — Vanna/Charm pressure building. Watch for delta-hedge driven moves.</div>'
        else:
            html += '<div style="font-size:0.7rem;color:#8B949E;margin-bottom:2px;">⏳ Vanna/Charm dormant — Far from expiry. Delta shifts gradual.</div>'

    if vanna is not None:
        try:
            v = float(vanna)
            if v > 0.5:
                html += f'<div style="font-size:0.7rem;color:#3FB950;margin-bottom:2px;">🟢 Vanna +{v:.2f}: Rally = vol crush. Buy spot on dips. Post-expiry vanna reversal risk if IV collapses.</div>'
            elif v < -0.5:
                html += f'<div style="font-size:0.7rem;color:#F85149;margin-bottom:2px;">🔴 Vanna {v:.2f}: Rally = vol expansion. Breakouts volatile. Hedge with call spreads.</div>'
            else:
                html += f'<div style="font-size:0.7rem;color:#8B949E;margin-bottom:2px;">🟡 Vanna {v:.2f}: Neutral spot-vol correlation.</div>'
        except:
            pass
    ...
    if charm is not None:
        try:
            c = float(charm)
            if c > 0.5:
                html += f'<div style="font-size:0.7rem;color:#3FB950;margin-bottom:2px;">🟢 Charm +{c:.2f}: Put support strengthening daily. Support level rises each day. Hold period can be longer (5-10 days).</div>'
            elif c < -0.5:
                html += f'<div style="font-size:0.7rem;color:#F85149;margin-bottom:2px;">🔴 Charm {c:.2f}: Put support eroding. Downside acceleration risk. Tighten stop. Short hold (1-3 days).</div>'
            else:
                html += f'<div style="font-size:0.7rem;color:#8B949E;margin-bottom:2px;">🟡 Charm {c:.2f}: Minimal theta-driven delta shift.</div>'
        except:
            pass

    if expected_move and expected_move > 0 and target and entry and entry > 0:
        move_to_target = abs(target - entry) / entry
        if move_to_target > expected_move * 3:
            html += f'<div style="font-size:0.7rem;color:#F85149;margin-bottom:2px;">🚀 FAST MOVE: Target distance {move_to_target:.1%} vs EM {expected_move:.1%}. Expected hold: 3-5 days (charm acceleration).</div>'
        elif move_to_target > expected_move * 2:
            html += f'<div style="font-size:0.7rem;color:#D29922;margin-bottom:2px;">⚡ MODERATE SPEED: Target >2x EM. Hold: 5-7 days.</div>'
    html += '</div>'

    # ── OI Heatmap Proxy ──
    html += '<div style="background:#0D1117;border-radius:6px;padding:8px 10px;margin-bottom:8px;">'
    html += '<div style="font-size:0.6rem;color:#8B949E;text-transform:uppercase;font-weight:600;margin-bottom:4px;">OI Heatmap Proxy</div>'
    if oi_call and oi_put:
        total_oi = oi_call + oi_put
        call_pct = oi_call / total_oi * 100
        put_pct = 100 - call_pct
        html += f'<div style="display:flex;height:14px;border-radius:3px;overflow:hidden;margin-bottom:4px;">'
        html += f'<div style="width:{call_pct:.0f}%;background:#3FB950;opacity:0.7;"></div>'
        html += f'<div style="width:{put_pct:.0f}%;background:#F85149;opacity:0.7;"></div></div>'
        html += f'<div style="display:flex;justify-content:space-between;font-size:0.65rem;color:#8B949E;">'
        html += f'<span>Calls {oi_call/1e6:.1f}M ({call_pct:.0f}%)</span><span>Puts {oi_put/1e6:.1f}M ({put_pct:.0f}%)</span></div>'
    else:
        html += '<div style="font-size:0.7rem;color:#484F58;">OI data not available (proxy source).</div>'

    if pc_ratio is not None:
        pcr_color = "#F85149" if pc_ratio < 0.6 else "#3FB950" if pc_ratio > 1.3 else "#8B949E"
        pcr_text = "Retail FOMO" if pc_ratio < 0.6 else "Hedging active" if pc_ratio > 1.3 else "Balanced"
        html += f'<div style="font-size:0.68rem;color:{pcr_color};margin-top:4px;">P/C Ratio: <b>{pc_ratio:.2f}</b> — {pcr_text}</div>'
    html += '</div>'

    # ── Volatility Forecast ──
    html += '<div style="background:#0D1117;border-radius:6px;padding:8px 10px;margin-bottom:8px;">'
    html += '<div style="font-size:0.6rem;color:#8B949E;text-transform:uppercase;font-weight:600;margin-bottom:4px;">Volatility Forecast</div>'
    if iv_rank is not None:
        try:
            ir = float(iv_rank)
            if ir < 35:
                html += f'<div style="font-size:0.7rem;color:#3FB950;">💤 IV Rank {ir:.0f}% — Options CHEAP. Ideal for buying calls/puts. Low premium = max asymmetric payoff.</div>'
            elif ir > 65:
                html += f'<div style="font-size:0.7rem;color:#F85149;">📢 IV Rank {ir:.0f}% — Options EXPENSIVE. Sell premium (covered calls/straddles) or wait for IV crush post-expiry.</div>'
            else:
                html += f'<div style="font-size:0.7rem;color:#8B949E;">⚖ IV Rank {ir:.0f}% — Fair value. No extreme vol edge.</div>'
        except:
            pass
    if expected_move:
        html += f'<div style="font-size:0.68rem;color:#484F58;margin-top:2px;">Expected Move: ±{expected_move:.1%} until expiry. Source: {source}</div>'
    html += '</div>'

    # ── MM Positioning Narrative ──
    mm_pos = opts.get("mm_positioning", "UNKNOWN")
    mm_rec = opts.get("mm_recommendation", "—")
    if mm_pos != "UNKNOWN":
        html += '<div style="background:#0D1117;border-radius:6px;padding:8px 10px;margin-bottom:8px;">'
        html += '<div style="font-size:0.6rem;color:#8B949E;text-transform:uppercase;font-weight:600;margin-bottom:4px;">Market Maker Positioning</div>'
        mm_color = "#D29922" if mm_pos == "PINNED" else "#3FB950" if mm_pos == "PUT_WALL" else "#F85149" if mm_pos == "CALL_WALL" else "#8B949E"
        html += f'<div style="font-size:0.8rem;color:{mm_color};font-weight:700;margin-bottom:2px;">{mm_pos}</div>'
        html += f'<div style="font-size:0.7rem;color:#8B949E;">{mm_rec}</div>'
        html += '</div>'
...
# ═══════════════════════════════════════════════════════════════════
# IHSG BANDARMOLOGY PANEL — Market Maker Intel
# ═══════════════════════════════════════════════════════════════════
def _render_ihsg_bandarmology(row):
    """Render IHSG-specific broker summary / bandarmology panel.
    Detects: Accumulation, Distribution, Cornering Supply, Crossing, Mark-up, Mark-down.
    Based on research: broker summary, top 5 net, foreign flow, volume pattern.
    """
    ticker = row.get("ticker", "?")
    px = row.get("price", 0)
    ...
    formation = row.get("formation", "NEUTRAL")
    direction = row.get("direction", "LONG")
    r5d = row.get("r5d", 0) or 0
    r20d = row.get("r20d", 0) or 0

    html = '<div class="ts-panel" style="border-color: #A371F740; margin-bottom: 8px;">'
    html += '<div class="ts-panel-title">🇮🇩 Bandarmology — Market Maker Intel (IHSG)</div>'

    phase = "NEUTRAL"
    phase_color = "#8B949E"
    phase_emoji = "⚪"
    phase_desc = "No clear bandar signal."

    acc = broker.get("real_accumulation", False)
    dist = broker.get("real_distribution", False)
    cross = broker.get("crossing_detected", False)
    corner = broker.get("cornering_supply", False)
    conf = broker.get("confidence", 0)
    ...
    if acc:
        ...
        phase_color = "#3FB950"
        phase_emoji = "📈"
        phase_desc = f"Genuine accumulation detected. Broker net buy dominant. Confidence {conf}%."
    elif dist:
        phase = "DISTRIBUSI"
        phase_color = "#F85149"
        phase_emoji = "📉"
        phase_desc = f"Genuine distribution detected. Broker net sell dominant. Confidence {conf}%."
    elif cross:
        phase = "CROSSING"
        phase_color = "#D29922"
        phase_emoji = "🎯"
        phase_desc = "High volume but flat price — possible wash trading (crossing). Wait for genuine breakout."
    elif corner:
        phase = "CORNERING SUPPLY"
        phase_color = "#58A6FF"
        phase_emoji = "🔒"
        phase_desc = "Volume drying up then spike. Tight float control. Possible accumulation before breakout."
    elif formation in ("BULLISH", "BULLISH_BIAS", "OVERSOLD") and r5d > 0.03:
        phase = "MARK-UP"
        phase_color = "#3FB950"
        phase_emoji = "🚀"
        phase_desc = "Price rising with momentum. Bandar likely in participation phase."
    elif formation in ("BEARISH", "BEARISH_BIAS", "OVERBOUGHT") and r5d < -0.03:
        phase = "MARK-DOWN"
        phase_color = "#F85149"
        phase_emoji = "💥"
        phase_desc = "Price falling with momentum. Distribution or panic selling."

    html += f'<div style="background:{phase_color}15;border:1px solid {phase_color}50;border-radius:6px;padding:8px 10px;margin-bottom:8px;">'
    html += f'<div style="font-size:0.85rem;color:{phase_color};font-weight:800;">{phase_emoji} {phase}</div>'
    html += f'<div style="font-size:0.72rem;color:#E6EDF3;margin-top:2px;">{phase_desc}</div>'
    html += '</div>'

    # Broker Summary Proxy Metrics
    html += '<div style="background:#0D1117;border-radius:6px;padding:8px 10px;margin-bottom:8px;">'
    html += '<div style="font-size:0.6rem;color:#8B949E;text-transform:uppercase;font-weight:600;margin-bottom:4px;">Broker Summary Proxy</div>'

    vol_ratio = broker.get("vol_ratio", 1.0)
    range_ratio = broker.get("range_ratio", 1.0)
    r5d_b = broker.get("r5d", 0)
    r20d_b = broker.get("r20d", 0)

    html += f'<div class="ts-grid-4">'
    html += f'<div class="ts-stat"><div class="ts-stat-label">Vol Ratio</div><div class="ts-stat-value" style="color:{"#F85149" if vol_ratio>1.5 else "#3FB950" if vol_ratio<0.7 else "#8B949E"};">{vol_ratio:.2f}x</div></div>'
    html += f'<div class="ts-stat"><div class="ts-stat-label">Range Ratio</div><div class="ts-stat-value" style="color:{"#D29922" if range_ratio<0.15 else "#8B949E"};">{range_ratio:.2f}</div></div>'
    html += f'<div class="ts-stat"><div class="ts-stat-label">R5D</div><div class="ts-stat-value" style="color:{"#3FB950" if r5d_b>0 else "#F85149"};">{r5d_b:+.1%}</div></div>'
    html += f'<div class="ts-stat"><div class="ts-stat-label">R20D</div><div class="ts-stat-value" style="color:{"#3FB950" if r20d_b>0 else "#F85149"};">{r20d_b:+.1%}</div></div>'
    html += f'</div>'

    if vol_ratio > 1.5 and range_ratio < 0.15:
        html += f'<div style="font-size:0.7rem;color:#F85149;margin-top:4px;">🚨 CROSSING ALERT: Vol spike {vol_ratio:.1f}x but range compressed {range_ratio:.2f} = wash trading likely.</div>'
    elif vol_ratio > 1.2 and r5d_b > 0.03 and not acc:
        html += '<div style="font-size:0.7rem;color:#D29922;margin-top:4px;">⚡ Volume expansion with price rise — check if backed by genuine broker buy or just retail FOMO.</div>'
    html += '</div>'

    # Bandar Strategy Guide
    html += '<div style="background:#0D1117;border-radius:6px;padding:8px 10px;margin-bottom:8px;">'
    html += '<div style="font-size:0.6rem;color:#8B949E;text-transform:uppercase;font-weight:600;margin-bottom:4px;">Bandar Strategy Guide</div>'
    html += '<div style="font-size:0.7rem;color:#8B949E;line-height:1.5;">'
    html += '• <b>Akumulasi:</b> Broker besar net buy bertahap, harga flat/sideways. Ikut beli di dekat support.<br>'
    html += '• <b>Mark-up:</b> Volume + harga naik signifikan. Bandar tarik ritel. Jangan chase terlalu tinggi.<br>'
    html += '• <b>Distribusi:</b> Broker besar net sell, harga flat/up tapi volume aneh. Waspada, jangan tangkap pisau jatuh.<br>'
    html += '• <b>Cornering:</b> Float dikontrol ketat, volume kering lalu spike. Breakout bisa 20-50% dalam hari.<br>'
    html += '• <b>Crossing:</b> Volume tinggi harga diam = broker saling jual-beli (wash). Hindari sampai breakout genuine.<br>'
    html += '</div></div>'

    # Foreign Flow Proxy (NBSA style)
    html += '<div style="background:#0D1117;border-radius:6px;padding:8px 10px;margin-bottom:8px;">'
    html += '<div style="font-size:0.6rem;color:#8B949E;text-transform:uppercase;font-weight:600;margin-bottom:4px;">Foreign Flow Proxy (NBSA)</div>'
    if r20d_b > 0.05 and vol_ratio > 1.0:
        html += '<div style="font-size:0.7rem;color:#3FB950;">🟢 Foreign Buy Proxy: Sustained uptrend + volume = likely foreign accumulation.</div>'
    elif r20d_b < -0.05 and vol_ratio > 1.0:
        html += '<div style="font-size:0.7rem;color:#F85149;">🔴 Foreign Sell Proxy: Sustained downtrend + volume = likely foreign distribution.</div>'
    else:
        html += '<div style="font-size:0.7rem;color:#8B949E;">⚪ Foreign Flow: Neutral / mixed signals.</div>'
    html += '</div>'
...
def page_ihsg():
    ...
    with st.expander("🔍 Quick Ticker Lookup", expanded=False):
        q_ticker = st.text_input("Enter ticker", "", key="ql_ihsg")
        if q_ticker:
            render_ticker_detail_comprehensive(q_ticker.upper().strip(), snap)

    ihsg_tickers = list(dict.fromkeys(FALLBACK_IHSG + list(IHSG_UNIVERSE.keys())))
    sim_results = snap.get("simulation_results", {}) or {}

    preview_rows = build_ticker_rows(ihsg_tickers, "ihsg", vix_now, prices=prices, ar=ar, snap=snap, sim_results=sim_results)
    acc_tickers = [r["ticker"] for r in preview_rows if r.get("broker",{}).get("real_accumulation")]
    dist_tickers = [r["ticker"] for r in preview_rows if r.get("broker",{}).get("real_distribution")]
    cross_tickers = [r["ticker"] for r in preview_rows if r.get("broker",{}).get("crossing_detected")]

    c1, c2, c3 = st.columns(3)
    with c1:
        st.markdown("<div style='font-size:0.68rem; color:#3FB950; text-transform:uppercase; font-weight:600; margin-bottom:3px;'>Akumulasi Real</div>", unsafe_allow_html=True)
        st.markdown("<div style='font-size:0.8rem; line-height:1.5;'>" + " · ".join(acc_tickers[:10]) + "</div>", unsafe_allow_html=True)
    with c2:
        st.markdown("<div style='font-size:0.68rem; color:#F85149; text-transform:uppercase; font-weight:600; margin-bottom:3px;'>Distribusi Real</div>", unsafe_allow_html=True)
        st.markdown("<div style='font-size:0.8rem; line-height:1.5;'>" + " · ".join(dist_tickers[:8]) + "</div>", unsafe_allow_html=True)
    with c3:
        st.markdown("<div style='font-size:0.68rem; color:#D29922; text-transform:uppercase; font-weight:600; margin-bottom:3px;'>Waspada Crossing</div>", unsafe_allow_html=True)
        st.markdown("<div style='font-size:0.8rem; line-height:1.5;'>" + " · ".join(cross_tickers[:8]) + "</div>", unsafe_allow_html=True)

    st.divider()
    tab_rr, tab_fr = st.tabs(["🎯 Risk Range + Bandarmology", "🔮 Front Run"])

    with tab_rr:
        st.markdown("### 🎯 Risk Range + Bandarmology — Edge Lokal")
        st.caption("TRR/LRR tanpa options. Bandarmology: akumulasi/distribusi/cornering/crossing dari broker summary proxy. BUY-ONLY market.")
        rows = build_ticker_rows(ihsg_tickers, "ihsg", vix_now, prices=prices, ar=ar, snap=snap, sim_results=sim_results)
        ...
        st.markdown(f"**{len(high_conv)} high conviction** · 🟢 {len(hc_longs)} Long · 🟡 {len(monitor)} monitor · ⚠️ {len(invalid)} filtered")
        sub_tab_l, sub_tab_m = st.tabs([f"🟢 Ready Long ({len(hc_longs)})", f"🟡 Monitor ({len(monitor)})"])
        with sub_tab_l: render_ticker_cards_v4(hc_longs)
        with sub_tab_m: render_ticker_cards_v4(monitor)
        if invalid:
            with st.expander(f"⚠️ Filtered ({len(invalid)} invalid)", expanded=False):
                render_invalid_cards(invalid)

    with tab_fr:
        st.markdown("### 🔮 Front Run — Indonesia Narrative")
        st.caption("Saham yang disiapkan untuk katalis: CPO, nickel, coal, banking, election, infrastructure. Bandarmology included.")
        chains = snap.get("supply_chain_chains", []) or []
        idn_tickers = []
        for chain in chains:
            if isinstance(chain, dict) and any(k in chain.get("trigger","").lower() for k in ["indonesia","resource","nationalism","cpo","nickel"]):
                for stage in chain.get("stages", []):
                    idn_tickers.extend(stage.get("tickers", []))
        idn_tickers = list(dict.fromkeys([t for t in idn_tickers if t.endswith(".JK") or t in IHSG_UNIVERSE]))
        if not idn_tickers:
            idn_tickers = ["BBRI.JK","BMRI.JK","BBCA.JK","ADRO.JK","ANTM.JK","AALI.JK","LSIP.JK"]
        fr_rows = build_ticker_rows(idn_tickers, "ihsg", vix_now, prices=prices, ar=ar, snap=snap, sim_results=sim_results)
        fr_actionable = filter_actionable(fr_rows, snap=snap)
        fr_longs = [r for r in fr_actionable if "LONG" in r.get("direction","")]
        st.markdown(f"**{len(fr_actionable)} setups** · 🟢 {len(fr_longs)} Long")
        render_ticker_cards_v4(fr_longs)
...
def page_alpha():
    ...
    st.divider()
    tab1, tab2, tab3, tab4 = st.tabs(["🎯 Unified Alpha", "🔮 Pure Front-Run", "📊 Vol & Squeeze", "🧠 Discovery & 100x"])

    with tab1:
        st.markdown("### 🎯 Unified Alpha — Quad Playbook × Bottleneck × Front-Run × Methodology")
        st.caption("Fusion of Hedgeye Quad playbook + Leopold bottleneck layers + COATUE rotation + Karsan vol + Thought Process thesis. Every ticker passes Walkforward gate (MC 100x) + Gatekeeper (8-gate). Only CHASE = ready to enter. WAIT = pullback needed. AVOID = broken setup.")
    ...
    with tab2:
        ...
        st.caption("Ticker-ticker murni yang muncul dari news catalyst, supply chain bottleneck, atau rumor front-run. Belum melalui full methodology fusion seperti tab Unified Alpha. Gunakan untuk watchlist awal.")
        fr = snap.get("front_run_candidates", []) or []
        if fr:
    ...
    with tab4:
        st.markdown("### 🧠 Discovery — 100x Potential Plays")
        st.caption("Tickers with asymmetric upside: bottleneck-driven, TAM expansion, or regime shift. Target: ratusan/ribuan persen. Referensi: SK Hynix (memory cycle 10x), SNDK (AI infrastructure 8x).")

        all_tickers = list(dict.fromkeys(FALLBACK_US + FALLBACK_CRYPTO + list(IHSG_UNIVERSE.keys())))
        rows = build_ticker_rows(all_tickers, "us_equity", vix_now, snap.get("gamma_data"), snap.get("greeks_data"), snap.get("news_narratives"), prices=prices, ar=ar, snap=snap, sim_results=sim_results)

        for r in rows:
            px = r.get("price", 999)
            mt = r.get("market_type", "us_equity")
            ev = r.get("ev_score", {}).get("score", 0) if isinstance(r.get("ev_score"), dict) else 0
            alpha = r.get("alpha_source", "")
            is_low_float = px < 5 if mt != "ihsg" else px < 1000
            if is_low_float and ev > 50 and alpha in ("bottleneck", "front_run", "leopold", "karsan_squeeze"):
                r["multibagger"] = True
                if "semiconductor" in r.get("alpha_thesis","").lower() or "ai" in r.get("alpha_thesis","").lower():
                    r["target_mcap_proxy"] = "Ref: SNDK/SK Hynix — 8x-12x during AI buildout super-cycle"
                elif "nickel" in r.get("alpha_thesis","").lower() or "cpo" in r.get("alpha_thesis","").lower() or "coal" in r.get("alpha_thesis","").lower():
                    r["target_mcap_proxy"] = "Ref: Commodity super-cycle — 5x-10x on supply bottleneck + export boom"
                elif "crypto" in mt or "btc" in r.get("ticker","").lower():
                    r["target_mcap_proxy"] = "Ref: Post-halving / ETF flow — 3x-5x cycle beta"
                else:
                    r["target_mcap_proxy"] = "Ref: Multi-bagger on TAM expansion + margin inflection — 5x-20x"
            else:
                r["multibagger"] = False

        mb_rows = [r for r in rows if r.get("multibagger")]
        mb_rows.sort(key=lambda x: x.get("ev_score",{}).get("score",0) if isinstance(x.get("ev_score"), dict) else x.get("quality_score",0), reverse=True)

        st.markdown(f"**{len(mb_rows)} multi-bagger candidates**")
        for r in mb_rows[:12]:
            t = r.get("ticker")
            px = r.get("price")
            ev = r.get("ev_score",{})
            thesis = r.get("alpha_thesis","Bottleneck / TAM expansion play")
            drive = r.get("alpha_source","bottleneck").replace("_"," ").title()
            mcap = r.get("target_mcap_proxy","Asymmetric upside")

            with st.container():
                st.markdown(
                    f'<div class="alpha-thesis-card" style="border-left-color:#FFD700;">'
                    f'<div style="display:flex;align-items:center;gap:8px;">'
                    f'<span style="font-size:0.9rem;font-weight:800;color:#E6EDF3;">{t}</span>'
                    f'<span style="background:#FFD70018;color:#FFD700;padding:2px 8px;border-radius:4px;font-size:0.65rem;font-weight:700;border:1px solid #FFD70040;">🚀 100x POTENTIAL</span>'
                    f'<span style="font-size:0.6rem;color:#484F58;">EV+ {ev.get("score",0):.0f} {ev.get("grade","")}</span>'
                    f'</div>'
                    f'<div class="alpha-thesis-sub"><b>Drive:</b> {drive} — {thesis}</div>'
                    f'<div class="alpha-thesis-sub"><b>Target Market Cap Proxy:</b> {mcap}</div>'
                    f'<div class="alpha-thesis-sub"><b>Current:</b> {_ffm(px, r.get("market_type","us_equity"))} · Entry: {_ffm(r.get("entry"), r.get("market_type","us_equity"))} · Stop: {_ffm(r.get("stop"), r.get("market_type","us_equity"))}</div>'
                    f'</div>', unsafe_allow_html=True
                )
                render_ticker_card_v4(r, expanded=False)
...
def page_us_stocks():
    ...
    c1, c2 = st.columns(2)
    with c1:
        st.markdown("<div style='font-size:0.68rem; color:#3FB950; text-transform:uppercase; font-weight:600; margin-bottom:3px;'>Hedgeye Overweight</div>", unsafe_allow_html=True)
        st.markdown("<div style='font-size:0.8rem; line-height:1.5;'>" + " · ".join(us_beli[:12]) + "</div>", unsafe_allow_html=True)
    with c2:
        st.markdown("<div style='font-size:0.68rem; color:#F85149; text-transform:uppercase; font-weight:600; margin-bottom:3px;'>Hedgeye Underweight / Avoid</div>", unsafe_allow_html=True)
        st.markdown("<div style='font-size:0.8rem; line-height:1.5;'>" + " · ".join(us_short[:10]) + "</div>", unsafe_allow_html=True)

    st.divider()
    tab_rr, tab_fr = st.tabs(["🎯 Hedgeye Risk Range", "🔮 Front Run"])

    with tab_rr:
        st.markdown("### 🎯 Hedgeye Risk Range — Trade · Trend · Tail")
        st.caption("Every ticker shows TRR/LRR with phase classification. Options/Greeks intel (Tier1 Alpha context) inside expander.")

        all_us = list(dict.fromkeys(FALLBACK_US + us_beli + us_short))
        sim_results = snap.get("simulation_results", {}) or {}
        rows = build_ticker_rows(all_us, "us_equity", vix_now, snap.get("gamma_data"), snap.get("greeks_data"), snap.get("news_narratives"), prices=prices, ar=ar, snap=snap, sim_results=sim_results)
        actionable = filter_actionable(rows, snap=snap)
        invalid = filter_invalid(rows)
        high_conv = filter_high_conviction(actionable)
        hc_longs, hc_shorts = split_long_short(high_conv)
        ...
        st.markdown(f"**{len(high_conv)} high conviction** · 🟢 {len(hc_longs)} Long · 🔴 {len(hc_shorts)} Short · 🟡 {len(monitor)} monitor · ⚠️ {len(invalid)} filtered")
        sub_tab_l, sub_tab_s, sub_tab_m = st.tabs([f"🟢 Ready Long ({len(hc_longs)})", f"🔴 Ready Short ({len(hc_shorts)})", f"🟡 Monitor ({len(monitor)})"])
        with sub_tab_l: render_ticker_cards_v4(hc_longs)
        with sub_tab_s: render_ticker_cards_v4(hc_shorts)
        with sub_tab_m: render_ticker_cards_v4(monitor)
        if invalid:
            with st.expander(f"⚠️ Filtered Out ({len(invalid)} invalid)", expanded=False):
                render_invalid_cards(invalid)

    with tab_fr:
        st.markdown("### 🔮 Front Run — Prepared for Next Move")
        st.caption("Tickers aligned with quad playbook, bottleneck chains, or catalyst setup. Front-run before the crowd. Options/Greeks intel included.")

        fr_tickers = list(dict.fromkeys(us_beli + [item.get("ticker","") for item in (snap.get("front_run_candidates",[]) or []) if isinstance(item, dict) and _classify_ticker_market(item.get("ticker",""))=="us_equity"]))
        fr_rows = build_ticker_rows(fr_tickers, "us_equity", vix_now, snap.get("gamma_data"), snap.get("greeks_data"), snap.get("news_narratives"), prices=prices, ar=ar, snap=snap, sim_results=sim_results)
        fr_actionable = filter_actionable(fr_rows, snap=snap)
        fr_invalid = filter_invalid(fr_rows)
        fr_longs, fr_shorts = split_long_short(fr_actionable)

        st.markdown(f"**{len(fr_actionable)} front-run setups** · 🟢 {len(fr_longs)} Long · 🔴 {len(fr_shorts)} Short")
        sub_tab_fl, sub_tab_fs = st.tabs([f"🟢 Front-Run Long ({len(fr_longs)})", f"🔴 Front-Run Short ({len(fr_shorts)})"])
        with sub_tab_fl: render_ticker_cards_v4(fr_longs)
        with sub_tab_fs: render_ticker_cards_v4(fr_shorts)
        if fr_invalid:
            with st.expander(f"⚠️ Filtered ({len(fr_invalid)} invalid)", expanded=False):
                render_invalid_cards(fr_invalid)
...
def page_forex():
    ...
    fx_tickers = list(dict.fromkeys(FALLBACK_FX + list(FOREX_PAIRS.keys())))
    sim_results = snap.get("simulation_results", {}) or {}

    cot_bull = [t for t in fx_tickers if _get_cot_proxy(t).get("signal")=="BULLISH"]
    cot_bear = [t for t in fx_tickers if _get_cot_proxy(t).get("signal")=="BEARISH"]
    c1, c2 = st.columns(2)
    with c1:
        st.markdown("<div style='font-size:0.68rem; color:#3FB950; text-transform:uppercase; font-weight:600; margin-bottom:3px;'>COT Bullish</div>", unsafe_allow_html=True)
        st.markdown("<div style='font-size:0.8rem; line-height:1.5;'>" + " · ".join(cot_bull[:10]) + "</div>", unsafe_allow_html=True)
    with c2:
        st.markdown("<div style='font-size:0.68rem; color:#F85149; text-transform:uppercase; font-weight:600; margin-bottom:3px;'>COT Bearish</div>", unsafe_allow_html=True)
        st.markdown("<div style='font-size:0.8rem; line-height:1.5;'>" + " · ".join(cot_bear[:10]) + "</div>", unsafe_allow_html=True)

    st.divider()
    tab_rr, tab_fr = st.tabs(["🎯 Risk Range + COT", "🔮 Front Run"])

    with tab_rr:
        st.markdown("### 🎯 Risk Range + COT — Institutional Edge")
        st.caption("TRR/LRR + Commitment of Traders (COT) non-commercial positioning. Options/Greeks intel if available. OI heatmap proxy included.")
        ...
        for r in rows:
            cot = _get_cot_proxy(r.get("ticker"))
            if cot and cot.get("signal") != "NEUTRAL":
                r["cot_signal"] = cot.get("signal")
                r["cot_net"] = cot.get("net_noncom")
                r["cot_confidence"] = min(100, abs(cot.get("net_noncom",0))/50000*100)
                if (cot.get("signal")=="BULLISH" and "LONG" in r.get("direction","")) or (cot.get("signal")=="BEARISH" and "SHORT" in r.get("direction","")):
                    r["quality_score"] = r.get("quality_score",0) + 15
                    r["quality_reasons"] = r.get("quality_reasons",[]) + ["COT align"]
        actionable = filter_actionable(rows, snap=snap)
        invalid = filter_invalid(rows)
        high_conv = filter_high_conviction(actionable)
        hc_longs, hc_shorts = split_long_short(high_conv)
        ...
        st.markdown(f"**{len(high_conv)} high conviction** · 🟢 {len(hc_longs)} Long · 🔴 {len(hc_shorts)} Short · 🟡 {len(monitor)} monitor")
        sub_tab_l, sub_tab_s, sub_tab_m = st.tabs([f"🟢 Ready Long ({len(hc_longs)})", f"🔴 Ready Short ({len(hc_shorts)})", f"🟡 Monitor ({len(monitor)})"])
        with sub_tab_l: render_ticker_cards_v4(hc_longs)
        with sub_tab_s: render_ticker_cards_v4(hc_shorts)
        with sub_tab_m: render_ticker_cards_v4(monitor)
    ...
    with tab_fr:
        st.markdown("### 🔮 Front Run — Macro & Carry Setup")
        st.caption("Forex pairs preparing for regime shift or carry trade revival. Options/Greeks intel if available. OI heatmap proxy included.")
        pb = _get_hedgeye_playbook(snap)
        fx_favor = [t for t in pb.get("beli",[]) if _classify_ticker_market(t)=="forex"]
        fr_rows = build_ticker_rows(fx_favor, "forex", vix_now, snap.get("gamma_data"), snap.get("greeks_data"), snap.get("news_narratives"), prices=prices, ar=ar, snap=snap, sim_results=sim_results)
        fr_actionable = filter_actionable(fr_rows, snap=snap)
        fr_longs, fr_shorts = split_long_short(fr_actionable)
        st.markdown(f"**{len(fr_actionable)} setups** · 🟢 {len(fr_longs)} Long · 🔴 {len(fr_shorts)} Short")
        sub_tab_fl, sub_tab_fs = st.tabs([f"🟢 Front-Run Long ({len(fr_longs)})", f"🔴 Front-Run Short ({len(fr_shorts)})"])
        with sub_tab_fl: render_ticker_cards_v4(fr_longs)
        with sub_tab_fs: render_ticker_cards_v4(fr_shorts)
...
def page_commodities():
    ...
    comm_tickers = list(dict.fromkeys(FALLBACK_COMM + list(COMMODITIES.keys())))
    sim_results = snap.get("simulation_results", {}) or {}

    cot_bull = [t for t in comm_tickers if _get_cot_proxy(t).get("signal")=="BULLISH"]
    cot_bear = [t for t in comm_tickers if _get_cot_proxy(t).get("signal")=="BEARISH"]
    c1, c2 = st.columns(2)
    with c1:
        st.markdown("<div style='font-size:0.68rem; color:#3FB950; text-transform:uppercase; font-weight:600; margin-bottom:3px;'>COT Bullish</div>", unsafe_allow_html=True)
        st.markdown("<div style='font-size:0.8rem; line-height:1.5;'>" + " · ".join(cot_bull[:10]) + "</div>", unsafe_allow_html=True)
    ...
    with tab_fr:
        st.markdown("### 🔮 Front Run — Supply Chain & Geopolitical")
        st.caption("Commodities preparing for supply shock or demand surge. Options/Greeks intel if available. OI heatmap proxy included.")
        pb = _get_hedgeye_playbook(snap)
        comm_favor = [t for t in pb.get("beli",[]) if _classify_ticker_market(t)=="commodity"]
        fr_rows = build_ticker_rows(comm_favor, "commodity", vix_now, snap.get("gamma_data"), snap.get("greeks_data"), snap.get("news_narratives"), prices=prices, ar=ar, snap=snap, sim_results=sim_results)
        fr_actionable = filter_actionable(fr_rows, snap=snap)
        fr_longs, fr_shorts = split_long_short(fr_actionable)
        st.markdown(f"**{len(fr_actionable)} setups** · 🟢 {len(fr_longs)} Long · 🔴 {len(fr_shorts)} Short")
        sub_tab_fl, sub_tab_fs = st.tabs([f"🟢 Front-Run Long ({len(fr_longs)})", f"🔴 Front-Run Short ({len(fr_shorts)})"])
...
def page_crypto():
    ...
    crypto_tickers = list(dict.fromkeys(FALLBACK_CRYPTO + list(CRYPTO.keys())))
    sim_results = snap.get("simulation_results", {}) or {}

    onchain_bull = []
    onchain_bear = []
    for t in crypto_tickers:
        oc = _get_onchain_proxy(t, prices)
        if oc.get("momentum") == "ACCUMULATING":
            onchain_bull.append(t)
        elif oc.get("momentum") == "DISTRIBUTING":
            onchain_bear.append(t)
    c1, c2 = st.columns(2)
    with c1:
        st.markdown("<div style='font-size:0.68rem; color:#3FB950; text-transform:uppercase; font-weight:600; margin-bottom:3px;'>On-Chain Accumulating</div>", unsafe_allow_html=True)
        st.markdown("<div style='font-size:0.8rem; line-height:1.5;'>" + " · ".join(onchain_bull[:10]) + "</div>", unsafe_allow_html=True)
    with c2:
        st.markdown("<div style='font-size:0.68rem; color:#F85149; text-transform:uppercase; font-weight:600; margin-bottom:3px;'>On-Chain Distributing</div>", unsafe_allow_html=True)
        st.markdown("<div style='font-size:0.8rem; line-height:1.5;'>" + " · ".join(onchain_bear[:10]) + "</div>", unsafe_allow_html=True)

    st.divider()
    tab_rr, tab_fr = st.tabs(["🎯 Risk Range + On-Chain", "🔮 Front Run"])

    with tab_rr:
        st.markdown("### 🎯 Risk Range + On-Chain — Whale & Funding Edge")
        st.caption("TRR/LRR + whale momentum + funding rate proxy. On-chain accumulation/distribution boosts conviction. Options/Greeks intel included.")
        ...
        for r in rows:
            oc = _get_onchain_proxy(r.get("ticker"), prices)
            if oc:
                r["onchain"] = oc
                whale = oc.get("whale_signal","HOLD")
                if whale == "BUY" and "LONG" in r.get("direction",""):
                    r["quality_score"] = r.get("quality_score",0) + 12
                    r["quality_reasons"] = r.get("quality_reasons",[]) + ["Whale accum"]
                elif whale == "SELL" and "SHORT" in r.get("direction",""):
                    r["quality_score"] = r.get("quality_score",0) + 12
                    r["quality_reasons"] = r.get("quality_reasons",[]) + ["Whale dist"]
        actionable = filter_actionable(rows, snap=snap)
        invalid = filter_invalid(rows)
        high_conv = filter_high_conviction(actionable)
        hc_longs, hc_shorts = split_long_short(high_conv)
    ...
    with tab_fr:
        st.markdown("### 🔮 Front Run — Halving / ETF / Regulatory Catalyst")
        st.caption("Crypto preparing for macro catalysts. High beta plays. Options/Greeks intel included.")
        pb = _get_hedgeye_playbook(snap)
        crypto_favor = [t for t in pb.get("beli",[]) if _classify_ticker_market(t)=="crypto"]
        fr_rows = build_ticker_rows(crypto_favor, "crypto", vix_now, snap.get("gamma_data"), snap.get("greeks_data"), snap.get("news_narratives"), prices=prices, ar=ar, snap=snap, sim_results=sim_results)
        fr_actionable = filter_actionable(fr_rows, snap=snap)
        fr_longs, fr_shorts = split_long_short(fr_actionable)
        st.markdown(f"**{len(fr_actionable)} setups** · 🟢 {len(fr_longs)} Long · 🔴 {len(fr_shorts)} Short")
        sub_tab_fl, sub_tab_fs = st.tabs([f"🟢 Front-Run Long ({len(fr_longs)})", f"🔴 Front-Run Short ({len(fr_shorts)})"])
...
with st.sidebar:
    ...
    page = st.radio("Navigation", [
        ...
        "🛢️ Commodities", "₿ Crypto", "🇮🇩 IHSG", "🌍 Global & EM", "📖 Themes", "📊 Portfolio Stress"
